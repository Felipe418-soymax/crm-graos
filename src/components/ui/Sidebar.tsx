...
export default function Sidebar({ user, isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        'fixed inset-y-0 left-0 z-40 w-72 bg-gray-900 flex flex-col transition-transform duration-300 ease-in-out',
        'md:static md:w-64 md:z-auto md:translate-x-0 md:transition-none',
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={cn('w-5 h-5 flex-shrink-0', isActive ? 'text-white' : 'text-gray-500 group-hover:text-gray-300')} size={18} />
